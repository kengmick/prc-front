<template>
  <div>
    ...
    <h1 class="main_red_text text-center">Edit Release</h1>
...
export default {
  ...
  async mounted() {
    const rel = await this.$strapi.find('releases', {
      id: this.$route.query.release,
    })
    this.rel = rel
    this.formValues = rel
  ...
  methods: {
    async submitForm() {
      if (this.profileImg) {
        console.log('this is pro image', this.profileImg)
        try {
      ...
      //   })
      //   try {
      //     const release = this.$strapi.upadte(
      //       'releases',
      ...
      try {
        ...
        // const [image, ...rest] = this.formValues
        const release = await this.$strapi.update(
          'releases',
          this.$route.query.release,
          { ...this.formValues }
        )
